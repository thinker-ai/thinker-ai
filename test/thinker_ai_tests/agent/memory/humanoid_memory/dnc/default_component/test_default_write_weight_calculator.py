...
class DefaultWriteWeightCalculatorTest(tf.test.TestCase):
    ...
    def test_write_weights_computation(self):
        ...
        memory_size = 4  # 确保 memory_size 与写入权重计算器一致
        ...
        # 实例化写入权重计算器，确保使用 softmax
        write_weight_calculator = DefaultWriteWeightCalculator(
            memory_size=memory_size,
            num_writes=num_writes,
            temperature=1.0  # 根据需要调整
        )

        # 手动计算 expected_allocation_weights 使用 softmax
        inverse_usage = 1.0 - prev_usage.numpy()  # [1,4]
        softmax_allocation = np.exp(inverse_usage / write_weight_calculator.temperature)
        softmax_allocation /= np.sum(softmax_allocation, axis=1, keepdims=True)  # [1,4]
        allocation_weights_expanded = softmax_allocation[:, np.newaxis, :]  # [1,1,4]
        ...
        allocation_gate_np = allocation_gate.numpy()
        write_gate_np = write_gate.numpy()

        expected_write_weights = write_gate_np[:, :, np.newaxis] * (
                allocation_gate_np[:, :, np.newaxis] * allocation_weights_expanded +
                (1 - allocation_gate_np[:, :, np.newaxis]) * write_content_weights_normalized
        )  # [1,2,4]
    ...
    def test_write_weights_with_zero_prev_usage(self):
        ...
        prev_usage = tf.zeros([batch_size, memory_size], dtype=tf.float32)  # [1, 4]
        ...
        expected_write_weights = write_gate_np[:, :, np.newaxis] * (
                allocation_gate_np[:, :, np.newaxis] * allocation_weights_expanded +
                (1 - allocation_gate_np[:, :, np.newaxis]) * write_content_weights_np
        )  # [1,2,4]
        ...
        # Check that write_weights sum to write_gate
    ...
    def test_write_weights_normalization(self):
        ...
        allocation_gate = tf.ones([batch_size, num_writes], dtype=tf.float32)
        write_gate = tf.ones([batch_size, num_writes], dtype=tf.float32)
        ...
        allocation_weights = self.write_weight_calculator.compute_allocation_weights(
            prev_usage)  # [batch_size, num_writes, memory_size]
    ...
    def test_write_content_weights_zero(self):
        ...
        # 期望的写入权重
        expected_write_weights = allocation_weights  # [1, 2, 4]
    ...
    def test_write_content_weights_non_normalized(self):
        """
        测试 write_weights 的计算在 write_content_weights 非标准化的情况下是否仍然保持非负。
        """
        batch_size = 2
        memory_size = 4
        num_writes = 2
        ...
        self.assertGreaterEqual(tf.reduce_min(write_weights).numpy(), 0.0, "write_weights should be non-negative")
