...
class TestSimpleStateMachine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.context_repo = DefaultStateMachineContextRepository.from_file(self.base_dir,
                                                                            'test_simple_state_machine_contexts.json',
                                                                            StateMachineContextBuilder(),
                                                                            self.definition_repo)
        # 读取状态机实例
        self.state_machine = self.context_repo.get("group_context","simple_context")
