...
class TestTestGPT(asynctest.TestCase):
    ...
    async def test_a_completion_batch_text(self):
        llm = get_llm()
        inputs:list[str]=[]
        inputs.append("我用session.request()向你发送消息，你才能记住我们之前交流的上下文是吗，能记住多少条上下文？")
        inputs.append("如果我不用session.request()向你发送消息，而是每次都新建一个request，你就不能获取我们之前交流的上下文，是吗？")
        result:list[str] = await llm.a_generate_batch(inputs)
