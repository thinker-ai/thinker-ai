...
function setActiveMenuItem(selectedButton) {
    // 移除所有按钮的 active 类
...
function showDetail(details) {
    ...
    document.getElementById('editor-container').style.display = 'none';
    document.getElementById('resource-container').style.display = 'none';
    detailContainer.innerHTML = `<div id="detail-content">${details}</div>`;
}

function showResourceConfig(button) {
    setActiveMenuItem(button);
    document.getElementById('editor-container').style.display = 'none';
    document.getElementById('detail-container').style.display = 'none';
...
function submitProblem() {
    // 提交问题描述逻辑，这里模拟提交后更新方案分解
    showSolution(); // 提交问题后自动显示方案分解菜单和目录树
}

function submitResourceConfig() {
    // 提交资源配置的逻辑，例如通过 Ajax 提交到服务器
    alert('资源配置已保存');
