...
from thinker_ai.configs.const import APP_HOME
...
def generate_repo(
    ...
    asyncio.run(company.run(n_round=n_round),debug=debug)
...
def startup(
    ...
    debug: bool = typer.Option(default=False, help="debug mode. Use it to debug project."),
    ...
    if idea is None:
        typer.echo("Missing argument 'IDEA'. Run 'thinker_ai --help' for more information.")
        raise typer.Exit()
...
# Reflected Code: https://github.com/geekan/MetaGPT/blob/main/thinker_ai/config2.py
...
def copy_config_to():
    ...
    target_path = APP_HOME / "config2.yaml"
