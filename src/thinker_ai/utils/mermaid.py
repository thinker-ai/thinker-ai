...
from thinker_ai.common.common import check_cmd_exists, awrite
...
async def mermaid_to_file(engine, mermaid_code, output_file_without_suffix, width=2048, height=2048) -> int:
    ...
    if engine == "nodejs":
        ...
        if engine == "playwright":
            from thinker_ai.utils.mmdc_playwright import mermaid_to_file

            return await mermaid_to_file(mermaid_code, output_file_without_suffix, width, height)
        elif engine == "pyppeteer":
            from thinker_ai.utils.mmdc_pyppeteer import mermaid_to_file

            return await mermaid_to_file(mermaid_code, output_file_without_suffix, width, height)
        elif engine == "ink":
            from thinker_ai.utils.mmdc_ink import mermaid_to_file
