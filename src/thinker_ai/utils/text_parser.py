...
class TextParser:
    ...
    def parse_blocks(cls, text: str):
        ...
        for block in blocks:
            ...
            if block.strip() and block.strip() != "":
                parts = [item.strip() for item in block.split("\n", 1)]
                if len(parts) == 2:
                    # 如果有足够的部分表示标题和内容，尝试将block的标题和内容分开
                    block_title, block_content = parts
                    # LLM可能出错，在这里做一下修正
                    if len(block_title) > 0 and block_title[-1] == ":":
                        block_title = block_title[:-1]
                else:
                    # 如果没有足够的部分表示标题和内容，则将整个block视为标题，内容为空
                    block_title = parts[0]
                    block_content = ""
    ...
    @classmethod
    def parse_data_with_mapping(cls, text:str, mapping:Dict[str, Tuple[Type[Any], Any]]) -> BaseModel:
        block_dict = cls.parse_blocks(text)
        parsed_data = {}
