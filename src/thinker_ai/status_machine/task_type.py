...
{status_definition_schema_simple}
 ...
 that the state can be decomposed into a sub-state machine at the next time!
    -In state machine schema, a state_def allows multiple actions to be configured, each action corresponds to a command, each action generates an event, and the next state generated by each event is determined in transitions.
