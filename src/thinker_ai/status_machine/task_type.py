...
{status_definition_schema_simple}
    ...
    -The key in the root path of your generated json file is the id of the state machine.
    -Generate only one state machine at a time based on the prompt
    -If the prompt want to generate a child state machine based on the state of a parent state machine, ensure that the key and name of the child state machine itself match the state_def_id and state_de_name of the corresponding state of the parent state machine.
