...
{status_definition_schema_simple}
    ...
    -The keys in the root path of your generated json file is the name of the state machine,if the state machine is a sub state machine,it is IMPORTANT that the name format MUST be {{root_state_machine_name}}.{{middle_state_machine_name}}.{{this_state_machine_name}} which the this_state_machine_name equals to the state_name of the middle_state_machine.
    -Generate only one state machine at a time based on the prompt
    -If you generate a child state machine of a parent state machine, ensure that the id and name of the child state machine itself match the state_def_name and state_def_name of the corresponding state of the parent state machine.
    ...
    -When reprogramming the exist status definition associated with the exist status instance, you have to look carefully at the current and historical states, and can only modify the state of state_def_name that is not in the current and historical states,and don't make unnecessary changes. 
    -Return ONLY the state machine with the requested name, do not also return other state machines.
