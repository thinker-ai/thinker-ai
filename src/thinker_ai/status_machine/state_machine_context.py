...
class BaseStateContext:
    def __init__(self, context_id: str, state_def: BaseStateDefinition):
...
class StateContext(BaseStateContext):
    def __init__(self, context_id: str, state_def: StateDefinition):
        super().__init__(context_id, state_def)
...
class StateContextBuilder(ABC):
    @staticmethod
    def build_state_context(state_def: StateDefinition, state_context_class_name: str,
                             context_id: Optional[str] = str(uuid.uuid4())) -> StateContext:
        raise NotImplementedError

    @staticmethod
    def build_terminal_state_context(state_def: BaseStateDefinition, state_context_class_name: str,
                                      context_id: Optional[str] = str(uuid.uuid4())) -> BaseStateContext:
        raise NotImplementedError

    @classmethod
    def build_composite_state_context(cls, state_def: "StateDefinition", state_context_class_name: str,
...
class StateContextDescription:
    def __init__(self,
                 ...
                 context_group_id: str,
                 context_id: Optional[str] = str(uuid.uuid4())
                 ):
        self.context_group_id: str = context_group_id
    ...
    def get_state_context(self,is_validate:bool) -> BaseStateContext:
        if is_validate:
            ...
            state_context = StateContextRegister.get(self.context_id)
            if not state_context:
                ...
                StateContextRegister.register(self.context_id, state_context)
        ...
        if isinstance(self.state_def, StateDefinition):
            if self.state_def.is_composite:
                state_context = (state_context_builder
                                 .build_composite_state_context(self.state_def,
                ...
                state_context = (
                    state_context_builder.build_state_context(self.state_def,
                                                               self.state_def.state_context_class_name,
                                                               self.context_id))
        else:
            state_context = (
                state_context_builder.build_terminal_state_context(self.state_def,
                                                                    self.state_def.state_context_class_name,
                                                                    self.context_id))
        return state_context
...
class DefaultStateContextBuilder(StateContextBuilder):
    @staticmethod
    def build_state_context(state_def: StateDefinition,
                             state_context_class_name: str,
                             context_id: Optional[str] = str(uuid.uuid4())) -> StateContext:
        return StateContext(context_id=context_id, state_def=state_def)

    @staticmethod
    def build_terminal_state_context(state_def: BaseStateDefinition,
                                      state_context_class_name: str,
                                      context_id: Optional[str] = str(uuid.uuid4())) -> BaseStateContext:
        return BaseStateContext(context_id=context_id, state_def=state_def)

    @classmethod
    def build_composite_state_context(cls, state_def: StateDefinition,
                                       state_context_class_name: str,
                                       state_machine_definition_repository: "StateMachineDefinitionRepository",
                                       state_machine_context_repository: "StateMachineRepository",
                                       context_group_id: str,
        ...
        if state_def.is_composite:
            return CompositeStateContext(context_id=context_id,
...
class StateMachine:
    def __init__(self, context_id: str,
        ...
        self.context_id = context_id
        self.context_group_id = context_group_id
    ...
    def on_event(self, event: Event):
        ...
        for transition in transitions:
            if transition.event == event.name and transition.source.name == self.current_state_context_des.state_def.name:
                ...
                self.state_machine_repository.set(self.context_group_id, self)
                return
        raise ValueError(
...
class CompositeStateContext(StateContext):
    def __init__(self, context_id: str, context_group_id: str,
    ...
    def get_state_machine(self) -> StateMachine:
        result_state_machine = self.state_machine_context_repository.get(self.context_group_id, self.context_id)
        if not result_state_machine:
            ...
            result_state_machine = StateMachine(context_id=self.context_id,
                                                context_group_id=self.context_group_id,
            ...
            self.state_machine_context_repository.set(self.context_group_id, result_state_machine)
...
class StateMachineContextBuilder:
    ...
    def new_from_group_def_json(cls, state_machine_def_group_name: str,
                                ...
                                state_machine_definition_repository: StateMachineDefinitionRepository,
                                state_machine_context_repository: StateMachineRepository,
                                context_group_id: str = str(uuid.uuid4()),
                                ) -> StateMachine:
        ...
        state_machine = cls.new_from_def(state_machine_def,state_machine_definition_repository,state_machine_context_repository,context_group_id)
    ...
    def new_from_def_name(cls, state_machine_def_group_name: str,
                          ...
                          state_machine_definition_repository: StateMachineDefinitionRepository,
                          state_machine_context_repository: StateMachineRepository,
                          context_group_id: str = str(uuid.uuid4())) -> StateMachine:
    ...
    def state_machine_to_dict(cls, state_machine: StateMachine) -> Dict[str, Any]:
        current_state_context = {
            "context_id": state_machine.current_state_context_des.context_id,
            "state_def_name": state_machine.current_state_context_des.state_def.name,
        }

        history_data = [
            {
                "context_id": context_des.context_id,
    ...
    @classmethod
    def state_machine_from_dict(cls, context_group_id: str, context_id: str, data: Dict[str, Any],
                                state_machine_definition_repository: StateMachineDefinitionRepository,
                                state_machine_context_repository: StateMachineRepository
                                ) -> StateMachine:
        state_machine_def = (state_machine_definition_repository
        ...
        current_state_context = StateContextDescription(
            ...
            state_machine_repository=state_machine_context_repository,
            context_group_id=context_group_id,
            context_id=current_state_context_date["context_id"])
    ...
    @classmethod
    def state_machine_from_json(cls, context_group_id: str, context_id: str,
                                json_text: str,
                                state_machine_definition_repository: StateMachineDefinitionRepository,
                                state_machine_context_repository: StateMachineRepository
                                ) -> "StateMachine":
        ...
        return cls.state_machine_from_dict(context_group_id, context_id, data, state_machine_definition_repository,
    ...
    def state_machine_group_from_dict(cls, data: Dict[str, Any],
        ...
        result = {}
        for context_group_id in data.keys():
            group_data = data.get(context_group_id)
            root_context = {}
            for state_machine_id in group_data.keys():
                state_machine_data = group_data.get(state_machine_id)

                state_machine = StateMachineContextBuilder.state_machine_from_dict(context_group_id,
                                                                                   state_machine_id,
                                                                                   state_machine_data,
                                                                                   state_machine_definition_repository,
                                                                                   state_machine_context_repository)
                root_context[state_machine_id] = state_machine
            result[context_group_id] = root_context
        return result
