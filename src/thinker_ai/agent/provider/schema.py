...
from thinker_ai.common.val_class import any_to_str, any_to_str_set, import_class
from thinker_ai.configs.const import MESSAGE_ROUTE_CAUSE_BY, MESSAGE_ROUTE_FROM, MESSAGE_ROUTE_TO, PRDS_FILE_REPO, \
    SYSTEM_DESIGN_FILE_REPO, TASK_FILE_REPO, MESSAGE_ROUTE_TO_ALL
from thinker_ai.repo_parser import DotClassInfo
from thinker_ai.utils.serialize import actionoutout_schema_to_mapping, actionoutput_mapping_to_str, \
...
class SerializationMixin(BaseModel, extra="forbid"):
    ...
    def __convert_to_real_type__(cls, value: Any, handler):
        ...
        if not isinstance(class_type, type):
            raise TypeError(f"{class_full_name} is not a class, found type {type(class_type)}")
...
class Message(BaseModel):
    ...
    def check_instruct_content(cls, ic: Any) -> BaseModel:
        if ic and isinstance(ic, dict) and "class" in ic:
            if "mapping" in ic:
                ...
                actionnode_class = import_class("ActionNode", "thinker_ai.agent.actions.action_node")  # avoid circular import
    ...
    def check_cause_by(cls, cause_by: Any) -> str:
        return any_to_str(cause_by if cause_by else import_class("UserRequirement", "thinker_ai.agent.actions.add_requirement"))
    ...
    def ser_instruct_content(self, ic: BaseModel) -> Union[dict, None]:
        ...
        if ic:
            ...
            ic_type = str(type(ic))
            if "<class 'thinker_ai.agent.actions.action_node" in ic_type:
