...
class SeleniumWrapper:
    ...
    user_1. Run the following command: pip install metagpt[selenium].
    ...
    async def _run_precheck(self):
        ...
        self._get_driver = await self.loop.run_in_executor(
            self.executor,
            lambda: _gen_get_driver_func(web_browser_engine_type, *self.launch_args, executable_path=self.executable_path),
        )
...
if __name__ == "__main__":
    ...
    async def main(url: str, *urls: str, **kwargs):
        return await SeleniumWrapper(web_browser_engine_type, **kwargs).run(url, *urls)
