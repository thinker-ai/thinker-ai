...
from thinker_ai.agent.rag.factories.base import GenericFactory
from thinker_ai.configs.config import config
from thinker_ai.configs.embedding_config import EmbeddingType
from thinker_ai.configs.llm_config import LLMType


class RAGEmbeddingFactory(GenericFactory):
    """Create LlamaIndex Embedding with ThinkerAI's embedding config."""
    ...
    def _resolve_embedding_type(self) -> EmbeddingType | LLMType:
        ...
        if config.client.api_type in [LLMType.OPENAI, LLMType.AZURE]:
            return config.client.api_type

        raise TypeError("To use RAG, please set your embedding in config2.yaml.")

    def _create_openai(self) -> OpenAIEmbedding:
        params = dict(
            api_key=config.embedding.api_key or config.client.api_key,
            api_base=config.embedding.base_url or config.client.base_url,
        )
    ...
    def _create_azure(self) -> AzureOpenAIEmbedding:
        params = dict(
            api_key=config.embedding.api_key or config.client.api_key,
            azure_endpoint=config.embedding.base_url or config.client.base_url,
            api_version=config.embedding.api_version or config.client.api_version,
        )
