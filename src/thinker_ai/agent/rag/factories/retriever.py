...
from thinker_ai.agent.rag.factories.base import ConfigBasedFactory
from thinker_ai.agent.rag.retrievers.base import RAGRetriever
from thinker_ai.agent.rag.retrievers.bm25_retriever import DynamicBM25Retriever
from thinker_ai.agent.rag.retrievers.chroma_retriever import ChromaRetriever
from thinker_ai.agent.rag.retrievers.es_retriever import ElasticsearchRetriever
from thinker_ai.agent.rag.retrievers.faiss_retriever import FAISSRetriever
from thinker_ai.agent.rag.retrievers.hybrid_retriever import SimpleHybridRetriever
from thinker_ai.agent.rag.schema import (
    BaseRetrieverConfig,
    BM25RetrieverConfig,
...
class RetrieverFactory(ConfigBasedFactory):
    ...
    def _build_faiss_index(self, config: FAISSRetrieverConfig, **kwargs) -> VectorStoreIndex:
        vector_store = FaissVectorStore(faiss_index=AnnoyIndex(config.dimensions, 'euclidean'))
