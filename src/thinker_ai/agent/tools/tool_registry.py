...
from thinker_ai.agent.tools.tool_convert import (
    convert_code_to_tool_schema,
    convert_code_to_tool_schema_ast,
)
from thinker_ai.agent.tools.tool_data_type import Tool, ToolSchema
from thinker_ai.common.logs import logger
from thinker_ai.configs.const import TOOL_SCHEMA_PATH
...
def register_tool(tags: list[str] = None, schema_path: str = "", **kwargs):
    ...
    def decorator(cls):
        ...
        if "thinker_ai" in file_path:
            # split to handle ../thinker_ai/thinker_ai/tools/... where only metapgt/tools/... is needed
            file_path = "thinker_ai" + file_path.split("thinker_ai")[-1]
