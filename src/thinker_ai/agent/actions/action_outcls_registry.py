...
def register_action_outcls(func):
    ...
    def decorater(*args, **kwargs):
        """
        arr example
            [<class 'thinker_ai.agent.actions.action_node.ActionNode'>, 'test', {'field': (str, Ellipsis)}]
        """
        arr = list(args) + list(kwargs.values())
        """
        outcls_id example
            "<class 'thinker_ai.agent.actions.action_node.ActionNode'>_test_{'field': (str, Ellipsis)}"
        """
