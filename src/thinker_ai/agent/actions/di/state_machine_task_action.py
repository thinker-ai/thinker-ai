...
class StateMachineTaskPlanAction(Action):
    ...
    async def run(self, goal: str, task_name, instruction: str,exec_logger: Memory) -> str:
        ...
        ```
        the exist state machine instances are here,they are related to the current state machine and are the scenario of the current state machine:
        ```json
        ...
        prompt = PROMPT_TEMPLATE.format(
            ...
            exist_status_instance=replace_curly_braces(state_machine_scenario_repository.group_to_json(goal)),
