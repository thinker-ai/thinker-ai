...
from thinker_ai.configs.const import GRAPH_REPO_FILE_REPO
from thinker_ai.repo_parser import CodeBlockInfo, DotClassInfo
from thinker_ai.utils.di_graph_repository import DiGraphRepository
from thinker_ai.utils.graph_repository import SPO, GraphKeyword, GraphRepository
...
class RebuildSequenceView(Action):
    ...
    def _get_full_filename(root: str | Path, pathname: str | Path) -> Path | None:
        ...
        Examples:
            If `root`(workdir) is "/User/xxx/github/ThinkerAI/thinker_ai", and the `pathname` is
            "thinker_ai/management/skill_manager.py", then the returned value will be
            "/User/xxx/github/ThinkerAI/thinker_ai/management/skill_manager.py"
        """
