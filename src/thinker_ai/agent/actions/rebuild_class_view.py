...
from thinker_ai.agent.provider.schema import UMLClassView
from thinker_ai.common.common import split_namespace, concat_namespace
from thinker_ai.common.logs import logger
from thinker_ai.configs.config import config
from thinker_ai.configs.const import (
    AGGREGATION,
    COMPOSITION,
...
from thinker_ai.repo_parser import DotClassInfo, RepoParser
from thinker_ai.utils.di_graph_repository import DiGraphRepository
from thinker_ai.utils.graph_repository import GraphKeyword, GraphRepository
...
class RebuildClassView(Action):
    ...
    def _diff_path(path_root: Path, package_root: Path) -> (str, str):
        ...
        Example:
            >>> _diff_path(path_root=Path("/Users/x/github/ThinkerAI"), package_root=Path("/Users/x/github/ThinkerAI/thinker_ai"))
            "-", "thinker_ai"

            >>> _diff_path(path_root=Path("/Users/x/github/ThinkerAI/thinker_ai"), package_root=Path("/Users/x/github/ThinkerAI/thinker_ai"))
            "=", "."
        """
    ...
    def _align_root(path: str, direction: str, diff_path: str) -> str:
        ...
        Example:
             _align_root(path="thinker_ai/software_company.py", direction="+", diff_path="ThinkerAI")
            "ThinkerAI/thinker_ai/software_company.py"

            _align_root(path="thinker_ai/software_company.py", direction="-", diff_path="thinker_ai")
            "software_company.py"
        """
        if direction == "=":
            return path
