...
from thinker_ai.common.val_class import is_coroutine_func, get_function_schema
from thinker_ai.env.base_env_space import BaseEnvObsParams, BaseEnvAction
from thinker_ai.env.env_api import WriteAPIRegistry, ReadAPIRegistry, EnvAPIAbstract
from thinker_ai.env.env_context import EvnContext
from thinker_ai.env.env_message import EnvMessage, is_send_to
...
class ExtEnv(BaseModel):
    ...
    async def write_thru_api(self, env_action: Union[str, EnvMessage, EnvAPIAbstract, list[EnvAPIAbstract]]):
        """execute through particular api of ExtEnv"""
        res = None
        if isinstance(env_action, EnvMessage):
...
class Environment(ExtEnv):
    ...
    context: EvnContext = Field(default_factory=EvnContext, exclude=True)
